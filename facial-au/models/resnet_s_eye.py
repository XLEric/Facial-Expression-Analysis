...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, img_size=224,dropout_factor = 1.):
        ...
        pool_kernel = int(img_size / 16)
        self.avgpool = nn.AvgPool2d(3, stride=1, ceil_mode=True)
...
if __name__ == "__main__":
    input = torch.randn([10, 3, 56,56])
    model = resnet18(False, num_classes=2, img_size=112)
    output = model(input)
    print(output.size())
