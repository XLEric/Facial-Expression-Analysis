...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, img_size=224,dropout_factor = 1.):
        ...
        self.fc1 = nn.Linear(2048, 256)
        sc = 16
        self.fc_branch1 = nn.Linear(256, 2*sc)
        self.fc_branch2 = nn.Linear(256, 6*sc)
        self.fc_branch3 = nn.Linear(256, 14*sc)
        self.fc_branch4 = nn.Linear(256, 2*sc)


        self.fc_branch5 = nn.Linear(2*sc, 2)
        self.fc_branch6 = nn.Linear(6*sc, 6)
        self.fc_branch7 = nn.Linear(14*sc, 14)
        self.fc_branch8 = nn.Linear(2*sc, 2)



        #---------------------------------

        self.fc2 = nn.Linear(16*24, num_classes)
    ...
    def forward(self, x):
        ...
        x1 = self.fc_branch1(x)
        x2 = self.fc_branch2(x)
        x3 = self.fc_branch3(x)
        x4 = self.fc_branch4(x)
        ...
        x1 = self.fc_branch5(x1)
        x2 = self.fc_branch6(x2)
        x3 = self.fc_branch7(x3)
        x4 = self.fc_branch8(x4)
...
if __name__ == "__main__":
    ...
    model = resnet18(False, num_classes=24, img_size=112)
    output = model(input)
    print(output.size())
